...
def run(
        ...
        classes= 0,  # 클래스 값 수정, Person = 0
        conf_thres=0.8,  # 임곗 값 수정 0.2 -> 0.8
        iou_thres=0.6,  # IOU 임곗 값 수정 0.45 -> 0.6
        max_det=200,  # 최대 탐지 수 수정 1000 -> 200
    ...
    person_num = len(det)
    print(f"{person_num} people detected.")
    return person_num
...
def webcam_capture():
    ...
    ret, frame = cap.read()
    if ret:
        cv2.imwrite(ROOT / 'data/images/capture.jpg', frame)
    else:
        print("웹캠을 찾을 수 없습니다.")
    cap.release()
...
def send_to_server(value):
    url = 'http://127.0.0.1:8000/api/update-value/'
    payload = {'value': value}
...
def main(opt):
    ...
    person_count = (run(**vars(opt)))
    send_to_server(person_count)
    time.sleep(60)
